...
/// <summary>
/// Scenario 15 — Outgoing service-token plumbing end-to-end. Closes the loop on Phase 1:
/// a typed HttpClient registered with AddServiceToken stamps a real JWT (issued by the
/// live auth service against a real Clients row) on outgoing calls to an in-process
/// downstream stub. Exercises OIDC discovery, token fetch, cache reuse on second call,
/// and RFC 6750 invalid_token-triggered retry. Downstream runs through TestServer — no
/// port binding, no certificate dance.
/// </summary>
...
public class ServiceTokenClientIntegrationTests(AppHostFixture fixture) : IntegrationTestBase(fixture)
{
    ...
    /// <summary>
    /// Logs in as the seeded admin and creates a Clients row with one (audience, scope)
    /// tuple. Returns the plaintext secret (one-time-display).
    /// </summary>
    ...
    /// <summary>
    /// Builds an IServiceCollection wired for end-to-end TokenClientLib: provider points
    /// at the running auth service; typed "downstream" client routes through the supplied
    /// handler so calls land in the in-process stub.
    /// </summary>
    ...
    /// <summary>
    /// Minimal-API app behind TestServer. Returns a handler for the typed-client's
    /// primary-handler slot, a thread-safe recorded-requests view, and a disposal hook.
    /// The responder is invoked per call (1-based index) unless
    /// <paramref name="attach401InvalidTokenHintOnFirstCall"/> is true and this is the
    /// first call, in which case it returns 401 + the RFC 6750 invalid_token hint.
    /// </summary>
        ...
        bool attach401InvalidTokenHintOnFirstCall = false)
    {
        ...
        app.MapGet("/items", (HttpContext ctx) =>
        {
            ...
            {
                // Set status + headers manually — Results.Unauthorized() can't attach
                // WWW-Authenticate on a minimal-API result.
