...
namespace AuthenticationService.Migrations.SqlServer;
...
/// a <c>DatabaseContext</c> wired with the SQL Server provider so it can diff the model
...
public sealed class DesignTimeFactory : IDesignTimeDbContextFactory<DatabaseContext>
{
    public DatabaseContext CreateDbContext(string[] args)
    {
        ...
        builder.UseSqlServer(
            "Server=design-time-placeholder;Database=AuthenticationService;User=sa;Password=placeholder;TrustServerCertificate=true",
            sql => sql.MigrationsAssembly(typeof(DesignTimeFactory).Assembly.GetName().Name));
